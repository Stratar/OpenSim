...
class Agent():  
    ...
    def save_weights(self):
        ...
        torch.save({
            ...
            }, base_path + '/SlimeVolley/policy.tar')
        
        torch.save({
            'model_state_dict': self.value.state_dict(),
            'optimizer_state_dict': self.value_optimizer.state_dict()
            }, base_path + '/SlimeVolley/value.tar')
        
    def load_weights(self):
        base_path = os.path.dirname(os.path.abspath(__file__))
        policy_checkpoint = torch.load(base_path + '/SlimeVolley/policy.tar')
        ...
        value_checkpoint = torch.load(base_path + '/SlimeVolley/value.tar')
...
def main():
    ...
    try:
        for i_episode in range(1, n_episode + 1):
            ...
            agent.save_weights()
            base_path = os.path.dirname(os.path.abspath(__file__))
            f = open(base_path + "/rewards.txt", "a+")
            f.write("Episode %d    " % i_episode)
            f.write("Reward  %d\r\n" % total_reward)
            f.close()
