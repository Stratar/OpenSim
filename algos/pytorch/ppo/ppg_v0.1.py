...
class Agent():  
    ...
    def save_weights(self, model_f):
        save_pol = model_f + '_policy.tar'
        torch.save({
            'model_state_dict': self.policy.state_dict(),
            'optimizer_state_dict': self.policy_optimizer.state_dict()
            }, save_pol)
        save_val = model_f + '_value.tar'
        torch.save({
            'model_state_dict': self.value.state_dict(),
            'optimizer_state_dict': self.value_optimizer.state_dict()
            }, save_val)
        
    def load_weights(self, model_f):
        load_pol = model_f + '_policy.tar'
        policy_checkpoint = torch.load(load_pol)
        ...
        load_val = model_f + '_value.tar'
        value_checkpoint = torch.load(load_val)
...
class Runner():
    def __init__(self, env, agent, render, training_mode, n_update, n_aux_update, max_action, logger):
        self.env = env
        self.agent = agent
        self.render = render
    ...
    def run_episode(self, eps):
        ...
        for _ in range(1, 1537):#Number of timesteps
            ...
            self.logger.store(VVals=reward)
            ep_true += true_reward
            ...
            if done: # only save EpRet / EpLen if trajectory finished
                ...
                self.logger.store(TrueRew=ep_true)
                ...
                self.logger.store(EpRet=total_reward, EpLen=eps_time)     
        return total_reward, eps_time, n_episodes
...
def train(model_file, load_after_iters, 
        restore_model_from_file=1, ac_kwargs=dict(), seed=0, 
    ...
    runner              = Runner(env, agent, render, training_mode, n_update, n_aux_update, max_action, logger)
    n_episodes = 0
    epoch = 0
    eps_time = 0

    #sync_params(agent.policy)
    #sync_params(agent.value)
    save_prefix = model_file[0:-5]
    #############################################     
    if load_weights:
    ...
    try:
        ...
        while True :
            ...
            total_reward, eps_time, n_episodes = runner.run_episode(n_episodes)

            #print('Episode {} \t t_reward: {} \t time: {} \t '.format(epoch, total_reward, eps_time))
            ...
            if epoch % save_after == 0 :
                ...
                model_f = os.path.normpath(
                    base_path + '/../../../' + save_prefix + '/models/' + save_prefix + "_afterIter_" + str(
                        epoch))
                agent.save_weights(model_f)
                logger.log("Saved model to file :{}".format(model_f))
                if n_episodes < 100:
                    size = n_episodes
                else:
                    size = 100
                asd = np.zeros((size, 3), dtype=np.int32)
                for i in range(size):
...
if __name__ == '__main__':
    ...
    train(model_file = args.env, load_after_iters=load_iters, 
        restore_model_from_file=args.restore,
