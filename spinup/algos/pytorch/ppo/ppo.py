...
def ppo(model_file, load_after_iters, restore_model_from_file=1, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, steps_per_epoch=4000, epochs=50, gamma=0.99, clip_ratio=0.2, pi_lr=3e-4,
        vf_lr=1e-3, train_pi_iters=80, train_v_iters=80, train_aux_iters=80, aux_iters=2, 
        lam=0.97, max_ep_len=1000, target_kl=0.01, logger_kwargs=dict(), save_freq=10, viz=False):
    """
    ...
    def save_weights(self, save_epoch):
        base_path = os.path.dirname(os.path.abspath(__file__))
        try:
    ...
    def load_weights(self, load_epoch):
        ...
        ac.pi.load_state_dict(value_checkpoint['model_state_dict'])
    ...
    for epoch in range(start_epoch, epochs):
        ...
        logger.log_tabular('EpTrue', average_only=True)
